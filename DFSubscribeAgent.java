...
public class DFSubscribeAgent extends Agent {
  ...
  protected void setup() {
		...
		templateSd.addProperties(new Property("panstwo", "Polska"));
		...
		addBehaviour(new SubscriptionInitiator(this, DFService.createSubscriptionMessage(this, getDefaultDF(), template, sc)) {
			protected void handleInform(ACLMessage inform) {
  			System.out.println("Agent "+getLocalName()+": Odebrano powiadomienie od moderatora katalogu");
  			try {
		  			...
		  			for (int i = 0; i < results.length; ++i) {
		  				...
		  				while (it.hasNext()) {
		  					...
		  					if (sd.getType().equals("prognoza-pogody")) {
	  							System.out.println("\nSerwis pogodowy znaleziony dla : Polska");

...
public class DFRegisterAgent extends Agent {
  ...
  protected void setup() {
  	...
  	System.out.println("Agent "+getLocalName()+" zarejestrowal serwis \""+serviceName+"\" typu \"Prognoza pogody\"");
  	try {
  		DFAgentDescription dfd = new DFAgentDescription();
  		dfd.setName(getAID());
  		ServiceDescription sd = new ServiceDescription();
  		sd.setName(serviceName);
  		sd.setType("prognoza-pogody");
  		// Agents that want to use this service need to "know" the weather-forecast-ontology
  		sd.addOntologies("prognoza-pogody-ontology");
  		// Agents that want to use this service need to "speak" the FIPA-SL language
  		sd.addLanguages(FIPANames.ContentLanguage.FIPA_SL);
  		sd.addProperties(new Property("panstwo", "Polska"));
  		dfd.addServices(sd);
